class MapService {
    ...
    async addIncident(incident) {
        try {
            ...
            console.log('Calculated offset:', offset, 'for marker:', location.id);
            ...
            // Add to clusters array before updating popup
            ...
            console.log('Cluster size for location:', key, 'is now:', this.markerClusters.get(key).length);
    ...
    addMarkerHoverEffects(container, el, popup, location) {
        ...
        container.addEventListener('click', (e) => {
            window.handleMarkerClick(e, container, el, popup, location);
        });
    }
    ...
    applyFilters() {
        ...
        this.markersMap.forEach((data, marker) => {
            const markerElement = marker.getElement();
            if (!markerElement) return;

            const weaponTypes = data.weaponTypes;
            if (!weaponTypes || weaponTypes.length === 0) {
                console.log('Marker has no weapon types:', data.id);
                return;
            }

            // Show marker only if there are active filters AND any of its weapon types match
            const shouldShow = this.activeFilters.size > 0 && 
                             weaponTypes.some(type => this.activeFilters.has(type));

            markerElement.style.display = shouldShow ? 'block' : 'none';
            
            // Also handle impact circle visibility if it exists
            const markerData = Array.from(this.markers.values())
                .find(m => m.marker === marker);
            
            if (markerData?.impactCircle) {
                ['gradientLayerId', 'outlineLayerId', 'rippleLayerId'].forEach(layerId => {
                    if (this.map.getLayer(markerData.impactCircle[layerId])) {
                        this.map.setLayoutProperty(
                            markerData.impactCircle[layerId],
                            'visibility',
