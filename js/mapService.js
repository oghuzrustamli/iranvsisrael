class MapService {
    ...
    async initializeMap() {
        try {
            ...
            // Get saved settings from local storage (only style and view mode)
            const savedSettings = this.loadMapSettings();
            
            // Default center coordinates between Iran and Israel
            const defaultCenter = [47.5, 32];
            const defaultZoom = 5;
            
            this.map = new mapboxgl.Map({
                container: 'map',
                style: savedSettings.style || 'mapbox://styles/mapbox/dark-v10',
            ...
            // Set 3D mode based on saved settings
            if (savedSettings.is3D) {
            ...
            this.updateMapStyleRadios(savedSettings.style);
            this.updateViewModeRadios(savedSettings.is3D);

            // Save only style and view mode settings when style changes
    ...
    async addCountryBorders() {
        try {
            ...
            if (this.map.getLayer('country-borders-glow')) this.map.removeLayer('country-borders-glow');
            if (this.map.getLayer('country-borders')) this.map.removeLayer('country-borders');
            if (this.map.getLayer('country-fills')) this.map.removeLayer('country-fills');
            if (this.map.getSource('country-borders')) this.map.removeSource('country-borders');
    ...
    async addIncident(incident) {
        try {
            ...
            // Create unique key for location grouping
            const locationKey = `${finalData.targetCity || finalData.location?.name || 'unknown'}_${new Date(finalData.timestamp).toDateString()}`;

            // Check if we already have a marker for this location and date
            if (this.markersMap.has(locationKey)) {
                console.log('Duplicate marker prevented:', locationKey);
                return;
            }
            ...
            this.markersMap.set(locationKey, markerData);
    ...
    // Load map settings from local storage (only style and view mode)
    loadMapSettings() {
        try {
    ...
    // Save current map settings to local storage (only style and view mode)
    saveMapSettings() {
        try {
            const settings = {
                style: this.map.getStyle().sprite,
                is3D: this.map.dragRotate.isEnabled()
            };
            localStorage.setItem('mapSettings', JSON.stringify(settings));
            console.log('Map settings saved:', settings);
        } catch (error) {
            console.error('Error saving map settings:', error);
        }
    }

    // Update map style radio buttons to match saved settings
    updateMapStyleRadios(savedStyle) {
    ...
    // Update view mode radio buttons to match saved settings
    updateViewModeRadios(is3D) {
        const viewModeRadios = document.querySelectorAll('input[name="viewMode"]');
        viewModeRadios.forEach(radio => {
            if ((radio.value === '3d' && is3D) || (radio.value === '2d' && !is3D)) {
                radio.checked = true;
            }
        });
    }
