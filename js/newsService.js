class NewsService {
        ...
        for (const item of articles) {
            try {
                ...
                if (analysis) {
                    ...
                    // Add only to feed, skip map
                    this.addNewsItemToFeed(id, newsItem, true);
                }
            } catch (error) {
    ...
    async loadNewsFromFirebase() {
        try {
            ...
            if (snapshot.exists()) {
                ...
                // Clear map markers but preserve manual ones
                this.mapService.clearAllIncidents(true);
    ...
    addNewsItemToFeed(id, item, skipMap = false) {
        try {
            ...
            if (!skipMap && item.locations && item.locations.length > 0) {
